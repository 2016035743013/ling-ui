...
//   }
// })
fs.readFile(`${__dirname}/Toast.jsx`, (err, data) => {
  if (err) {
    console.log(err)
  } else {
    const fileData = data.toString()
    const res = fileData.match(/Toast.propTypes = {[\S|\d|\D|\s|\W|\w]+\}/g) //+ 正则表达是模糊匹配proptypes
    const defaultRes = fileData.match(/Toast.defaultProps = {[\S|\d|\D|\s|\W|\w]+defaultend/g)
    const defaultValMap = {}
    if (defaultRes) {
      const defaultProps = defaultRes[0].replace(/(Toast.defaultProps|\{|\}|=|\n|\r|\s|\/\/defaultend)/g, '').split(',')
    ...
    if (res) {
      const propstr = res[0].replace(/(Toast.propTypes|\{|\}|=|\n|\r|\s)/g, '').split('end')
      const obj = {}
      console.log(propstr)
      propstr.map(item => {
