...
fs.readFile(`${__dirname}/index.jsx`, (err, data) => {
  if (err) {
    ...
    const res = fileData.match(/Alert.propTypes = {[\S|\d|\D|\s|\W|\w]+\}/g) //+ 正则表达是模糊匹配proptypes
    const defaultRes = fileData.match(/Alert.defaultProps = {[\S|\d|\D|\s|\W|\w]+defaultend/g)
    const defaultValMap = {}
    if (defaultRes) {
      const defaultProps = defaultRes[0].replace(/(Alert.defaultProps|\{|\}|=|\n|\r|\s|\/\/defaultend)/g, '').split(',')
    ...
    if (res) {
      const propstr = res[0].replace(/(Alert.propTypes|\{|\}|=|\n|\r|\s)/g, '').split('end')
      const obj = {}
      propstr.map(item => {
